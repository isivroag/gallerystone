...
if ($folio != "") {
    ...
    $consultatmp = "SELECT * FROM cxp WHERE tokenid='$tokenid' and estado='0' and tipo='CXPHERRAMIENTA' and estado_cxp=1 ORDER BY folio_cxp DESC LIMIT 1";
    ...
    if ($resultadotmp->rowCount() >= 1) {
        ...
        $fecha = date('Y-m-d');
        $consultatmp = "INSERT INTO cxp (tokenid,fecha,fecha_limite,tipo,subtotal,iva,total,saldo) VALUES('$tokenid','$fecha','$fecha','CXPHERRAMIENTA','0','0','0','0')";
        $resultadotmp = $conexion->prepare($consultatmp);
        $resultadotmp->execute();


        $consultatmp = "SELECT * FROM cxp WHERE tokenid='$tokenid' and estado='0' and tipo='CXPHERRAMIENTA' ORDER BY folio_cxp DESC LIMIT 1";
        $resultadotmp = $conexion->prepare($consultatmp);
        $resultadotmp->execute();
        $datatmp = $resultadotmp->fetchAll(PDO::FETCH_ASSOC);
    }
                            ...
                            <div class="col-lg-12 mx-auto">
                                <div class="table-responsive" style="padding:5px;">
                                    <table name="tablaDetIndes" id="tablaDetIndes" class="table table-sm table-striped table-bordered table-condensed text-nowrap mx-auto" style="width:100%;font-size:15px">
                                        ...
                                        <tbody>
                                            <?php
                                            $consultadeto = "SELECT * FROM vdetallecxp_herramienta where folio_cxp='$folio' and estado_reg=1 order by id_reg";
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    <td><?php echo $rowdet['cant_her'] ?></td>
                                                    <td><?php echo $rowdet['costo_her'] ?></td>
...
<script src="fjs/compraherramienta.js?v=<?php echo (rand()); ?>"></script>
