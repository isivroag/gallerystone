    ...
    columnDefs: [
      {
        ...
        data: null,
        defaultContent:
          "<div class='text-center'><div class='btn-group'><button class='btn btn-sm btn-success bntSelOrden'><i class='fas fa-hand-pointer'></i></button></div></div>",
      },
    ],
    ...
    if (folio.length != 0 && iddes.length != 0 && cantidadi.length != 0) {
      $.ajax({
        ...
        success: function (data) {
          ...
          tablaDetIndes.row
            .add([id_reg, id_des, tipo, clave_des, nom_des, cantidad, obs])
            .draw()
