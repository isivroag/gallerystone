...
$(document).ready(function () {
    ...
    tablaVis = $('#tablaV').DataTable({
        ...
        rowCallback: function (row, data) {
            ...
            if (data[3] == 'Entrada') {
                ...
                $($(row).find('td')[3]).addClass('bg-gradient-green')
                //$($(row).find('td')['3']).text('PENDIENTE')
              } else if (data[3] == 'Salida') {
                //$($(row).find("td")[3]).css("background-color", "blue");
                $($(row).find('td')[3]).addClass('bg-gradient-purple')
                //$($(row).find('td')['3']).text('ENVIADO')
              } else if (data[3] == 'Inventario Inicial') {
                //$($(row).find("td")[3]).css("background-color", "success");
                $($(row).find('td')[3]).addClass('bg-gradient-primary')
                //$($(row).find('td')['6']).text('ACEPTADO')
              }else if(data[3] == 'Entrada Can'){
                $($(row).find('td')[3]).addClass('bg-gradient-danger')
              }
              else if(data[3] == 'Salida Can'){
                $($(row).find('td')[3]).addClass('bg-gradient-danger')
              } else if(data[3] == 'Pedacería'){
                $($(row).find('td')[3]).addClass('bg-gradient-orange')
              }else if(data[3] == 'Cambio'){
                $($(row).find('td')[3]).addClass('bg-gradient-yellow text-white')
              }else if(data[3] == 'Corte'){
                $($(row).find('td')[3]).addClass('bg-gradient-info text-white')
              }
    ...
    $('#btnBuscar').click(function () {
        ...
        window.location.href="inventariomovmat.php?fechaini="+inicio
